	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.nongchangName" type="text" placeholder="农场名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('nongchang','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('nongchang','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='nongchangTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in nongchangTypesList" :key="index" :class="nongchangTypes===item.codeIndex?'tabActive':''"
                                  @tap="nongchangTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <block v-for="(product,index) in list" :key="index">
            <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
                ...
                <image
                        ...
                        mode="aspectFill" :src="product.nongchangPhoto?baseUrl+product.nongchangPhoto.split(',')[0]:''">
                </image>
                <view class="list-item-body" style="display: flex;flex-wrap: wrap;">
                    <view
                            ...
                            class="title">{{product.nongchangName}}
                    </view>
                    <view v-if='product.nongchangNewMoney'
                          ...
                          class="title">{{product.nongchangNewMoney}}
                    </view>
                    <view v-else-if='product.nongchangValue'
                          :style='{"padding":"0 24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                          class="title">{{product.nongchangValue}}
                    </view>
                </view>
                <view style="display: flex;justify-content: space-between;">
                    <text v-if="isAuth('nongchang','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('nongchang','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 农场类型 字段 字典表查询方法
            async nongchangTypesSelect(){
                let nongchangTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'nongchang_types',
                });
                nongchangTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.nongchangTypesList = nongchangTypes.data.list
            },
            //当前表的 农场类型字段类别搜索
			nongchangTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.nongchangTypes != 0) {
                    params.nongchangTypes = this.nongchangTypes
                }
                let _this = this
                let res = await this.$api.page(`nongchang`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('nongchang', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.nongchangShenheTime=shijian;
                            await _this.$api.update(`nongchang`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.nongchangName) {
                    searchForm['nongchangName'] = this.searchForm.nongchangName
                }

                let res = await this.$api.page(`nongchang`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.nongchangContent!=null){
                        item.nongchangContent = item.nongchangContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");

			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.nongchangName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="xiangqing">
					...
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app" :style="detail.nongchangTurang < detail.nongchangTr?'color: red;font-weight: 900;':''">
								{{detail.nongchangTurang}}
							</view>
						</view>
					</view>
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app" :style="detail.nongchangWenshidu < detail.nongchangWsd?'color: red;font-weight: 900;':''">
								{{detail.nongchangWenshidu}}
							</view>
						</view>
					</view>
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app" :style="detail.nongchangFengshu < detail.nongchangFs?'color: red;font-weight: 900;':''">
								{{detail.nongchangFengshu}}
							</view>
						</view>
					</view>
					<view class="xiangqing">
					...
					<view class="yinyuenei uni-padding-wrap uni-common-mt">
						<view>
							<video id="myVideo" style="preload:auto;margin: 20rpx;border-radius: 20rpx;" muted
								   :src="baseUrl+detail.nongchangVideo" controls></video>
						</view>
					</view>
				</view>
				<!-- 简介 -->
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.nongchangContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.nongchangId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getnongchangCollection()
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('nongchang', this.nongchangId);
                    this.detail = res.data
					this.detail.nongchangContent = this.detail.nongchangContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.nongchangPhoto ? this.detail.nongchangPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getnongchangCollection() {
					let params = {
						page: 1,
						limit: 1,
						nongchangId: this.nongchangId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`nongchangCollection`, params);
					if (res.data.list.length == 1) {
						let nongchangCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('nongchangCollection', JSON.stringify([nongchangCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getnongchangCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('nongchangCollection', {
									yonghuId: _this.user.id,
									nongchangId: _this.detail.id,
									nongchangCollectionTypes:1
								});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('nongchang', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('nongchangYuyue', {
									yonghuId: _this.user.id,
									nongchangId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/nongchangYuyue/list');
							}
						}
					});
				},

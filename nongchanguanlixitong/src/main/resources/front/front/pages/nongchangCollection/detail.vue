				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.nongchangCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.nongchangCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('nongchangCollection', this.nongchangCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.nongchangCollectionPhoto ? this.detail.nongchangCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('nongchangCollectionYuyue', {
									yonghuId: _this.user.id,
									nongchangCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/nongchangCollectionYuyue/list');
							}
						}
					});
				},

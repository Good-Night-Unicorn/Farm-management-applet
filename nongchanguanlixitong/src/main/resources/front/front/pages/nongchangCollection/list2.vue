	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.nongchangName" type="text" placeholder="农场"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('nongchangCollection','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
        ...
        <block v-for="(product,index) in list" :key="index">
            <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
                ...
                <view style="display: flex;justify-content: space-between;">
                    <text v-if="isAuth('nongchangCollection','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('nongchangCollection','删除')" class="cuIcon-delete"
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 类型 字段 字典表查询方法
		  	let nongchangCollectionTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'nongchang_collection_types'
		  	});
		  	this.nongchangCollectionTypesList = nongchangCollectionTypes.data.list;
			nongchangCollectionTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
            this.categoryList = nongchangCollectionTypes.data.list
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`nongchangCollection`, params);
			...
			onDetailTap(item) {
				this.$utils.jump(`../nongchang/detail?id=${item.nongchangId}`)
			},
			// 修改
			onUpdateTap(id) {
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`nongchangCollection`, {
                                id:id,
                            });
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('nongchangCollection', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.nongchangCollectionShenheTime=shijian;
                            await _this.$api.update(`nongchangCollection`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.page(`nongchangCollection`, searchForm);
				// 如果是第一页数据置空
